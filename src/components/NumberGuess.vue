<template>
  <div
    class="flex items-center justify-center h-screen bg-gradient-to-br from blue-100 to-indigo-200"
  >
    <div class="max-w-md mx-auto bg-white p-8 border rounded-2xl shadow-lg animaate-fade-in">
      <h2 class="text-3xl font-extrabold text-indigo-700 mb-6 text-center">Number Guessing Game</h2>
      <p class="text-sm text-gray-400 font-semibold mb-6">
        Guess the number between 1 - 50. You have 10 chance to guess the right number.
      </p>
      ...
      <transition name="fade">
        <p class="mt-6 text-center font-medium text-lg text-gray-700" v-if="message">
          {{ message }}
        </p>
      </transition>
      <p class="text-sm text-gray-400 text-center">{{ guesses?.length }} from 5 guesses</p>
      ...
      <div class="flex justify-end">
        <button @click="resetGame" class="mt-6 text-sm text-indigo-500 hover:underline transition">
          Reset
        </button>
      </div>
    </div>
