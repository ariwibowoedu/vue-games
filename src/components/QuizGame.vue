<template>
  <div
    class="flex items-center justify-center h-screen bg-gradient-to-br from-yellow-100 to-amber-200"
  >
    <div class="bg-white p-6 rounded-xl shadow-lg max-w-md w-full animate-fade-in">
      <h2 class="text-2xl font-bold text-center text-amber-700 mb-4">🧠 Quiz Game</h2>
      ...
      <div v-else-if="loading" class="text-center text-gray-600">🔄 Getting Task...</div>
        ...
        <div class="grid">
          <transition-group name="scale-fade" tag="div">
            <button
              ...
              :class="{
                'bg-green-300 scale-105': selected === option && isCorrect(option),
                'bg-red-300 scale-105': selected === option && !isCorrect(option),
      ...
      <div v-else class="text-center">
        <p class="text-xl font-semibold text-gray-700 mb-2">🎉 Quiz finished!</p>
        <p class="text-lg text-gray-600">
          Your score is {{ score }} from {{ questions.length }} quetions.
        </p>
        <button @click="resetQuiz" class="mt-4 text-amber-600 hover:underline text-sm">
          🔄 Try Again!
        </button>
      </div>
    </div>
...
const quizFinished = computed(() => currentIndex.value >= questions.value.length)
